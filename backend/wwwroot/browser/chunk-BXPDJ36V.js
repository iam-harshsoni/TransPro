...
<div class="page-header">\r
    ...
    <div class="page-title">Trips</div>\r
    <div class="page-subtitle">Manage transport trips and track their status</div>\r
</div>\r
\r
<div class="card">\r
\r
    <p-toolbar class="table-toolbar">\r
        <ng-template #start>\r
            <p-select [(ngModel)]="selectedStatus" [options]="statusOptions" optionLabel="label" optionValue="value"\r
                placeholder="Filter by Status" (onChange)="onStatusFilter()" class="ml-2" />\r
        </ng-template>\r
        <ng-template #end>\r
            <p-button label="New Trip" icon="pi pi-plus" (onClick)="navigateToCreate()" />\r
        </ng-template>\r
    </p-toolbar>\r
\r
    <p-table #dt [value]="trips" [loading]="isLoading()" [lazy]="true" (onLazyLoad)="onLazyLoad($event)"\r
        [totalRecords]="totalRecords" [paginator]="true" [rows]="pageSize" [rowsPerPageOptions]="[10, 25, 50]"\r
        [rowHover]="true" dataKey="id" [showCurrentPageReport]="true"\r
        currentPageReportTemplate="Showing {first} to {last} of {totalRecords} trips"\r
        [tableStyle]="{ 'min-width': '70rem' }">\r
\r
        <ng-template pTemplate="caption">\r
            <div class="table-caption">\r
                <span class="table-title">Trip List</span>\r
...
\r
        <ng-template pTemplate="header">\r
            <tr>\r
                ...
                <th style="min-width: 5rem">#</th>\r
                <th style="min-width: 12rem">Customer</th>\r
                <th style="min-width: 12rem">Driver</th>\r
                <th style="min-width: 10rem">Truck</th>\r
                <th style="min-width: 14rem">Route</th>\r
                <th style="min-width: 11rem">Departure</th>\r
                <th style="min-width: 9rem">Status</th>\r
            </tr>\r
        </ng-template>\r
\r
        <ng-template pTemplate="body" let-trip>\r
            <tr>\r
                <td>\r
                    <div class="row-actions">\r
                        @if (trip.status === 'Planned' || trip.status === 'InTransit') {\r
                        <p-button icon="pi pi-times-circle" severity="danger" [rounded]="true" [outlined]="true"\r
                            size="small" pTooltip="Cancel Trip" tooltipPosition="top" (onClick)="confirmCancel(trip)" />\r
                        }\r
                    </div>\r
                </td>\r
                <td>\r
                    <span class="breadcrumb-link trip-id cursor-pointer" pTooltip="View Details" tooltipPosition="top"\r
                        (click)="viewDetail(trip)">#{{ trip.id }}</span>\r
                </td>\r
                <td>\r
                    <span class="row-name">{{ trip.customerName }}</span>\r
                </td>\r
                <td>{{ trip.driverName }}</td>\r
                <td>\r
                    <span class="plate-number">{{ trip.truckPlate }}</span>\r
                </td>\r
                <td>\r
                    <!--\r
                        Route shown as Origin \u2192 Destination with arrow.\r
                        Makes it visually scannable at a glance.\r
                    -->\r
                    <div class="trip-route-cell">\r
                        <span>{{ trip.origin }} </span>\r
                        ->\r
                        <span> {{ trip.destination }}</span>\r
                    </div>\r
                </td>\r
                <td>\r
                    {{ trip.departureDate | date: 'dd MMM yyyy' }}\r
                </td>\r
                <td>\r
            ...
            <tr>\r
                <td colspan="8">\r
                    <div class="empty-state">\r
                        ...
                        <span>No trips found</span>\r
                        <small>Create your first trip to get started</small>\r
                    </div>\r
                </td>\r
            </tr>\r
        </ng-template>\r
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(TripListComponent, { className: "TripListComponent", filePath: "app/features/trips/pages/trip-list/trip-list.component.ts", lineNumber: 42 });
})();
export {
  TripListComponent
};
//# sourceMappingURL=chunk-BXPDJ36V.js.map
