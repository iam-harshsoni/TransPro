...
<div class="page-header">\r
    <div class="breadcrumb-trail">\r
        ...
        <a class="breadcrumb-link" (click)="onCancel()">Routes</a>\r
        <i class="pi pi-angle-right"></i>\r
        <span>{{ isEditMode() ? 'Edit Route' : 'New Route' }}</span>\r
    </div>\r
    <div class="page-title">{{ isEditMode() ? 'Edit Route' : 'New Route' }}</div>\r
</div>\r
\r
<div class="form-card">\r
    <div class="card-header">\r
        <span class="card-title">Route Information</span>\r
    </div>\r
\r
    <div class="card-body">\r
        <form [formGroup]="form" (ngSubmit)="onSubmit()">\r
\r
            <div class="form-section-title">Route Details</div>\r
\r
            <div class="form-grid-2">\r
                <div class="form-field">\r
                    <label class="form-label">\r
                        Origin <span class="required-star">*</span>\r
                    </label>\r
                    <input pInputText formControlName="origin" placeholder="e.g. Mumbai"\r
                        [class.input-error]="isInvalid('origin')" />\r
                    @if (isInvalid('origin')) {\r
                    <small class="error-msg">\r
                        @if (form.get('origin')?.hasError('required')) {\r
                        Origin is required.\r
                        }\r
                    </small>\r
                    }\r
                </div>\r
\r
                <div class="form-field">\r
                    <label class="form-label">\r
                        Destination <span class="required-star">*</span>\r
                    </label>\r
                    <input pInputText formControlName="destination" placeholder="e.g. Delhi"\r
                        [class.input-error]="isInvalid('destination')" />\r
                    @if (isInvalid('destination')) {\r
                    <small class="error-msg">\r
                        @if (form.get('destination')?.hasError('required')) {\r
...
\r
                <div class="form-field">\r
                    <label class="form-label">\r
                        Distance (km) <span class="required-star">*</span>\r
                    </label>\r
\r
                    <p-inputnumber formControlName="distanceKm" [min]="0.1" [minFractionDigits]="1"\r
                        [maxFractionDigits]="2" suffix=" km" placeholder="Enter distance"\r
                        [class.input-error]="isInvalid('distanceKm')" styleClass="w-full" />\r
                    @if (isInvalid('distanceKm')) {\r
                    <small class="error-msg">\r
                        @if (form.get('distanceKm')?.hasError('required')) {\r
                        Distance is required.\r
                        }\r
                        @if (form.get('distanceKm')?.hasError('min')) {\r
                        Distance must be at least 0.1 km.\r
                        }\r
                    </small>\r
                    }\r
                </div>\r
\r
                <div class="form-field">\r
                    <label class="form-label">\r
                        Estimated Duration (hours) <span class="required-star">*</span>\r
                    </label>\r
\r
                    <p-inputnumber formControlName="estimatedHours" [min]="0.1" [minFractionDigits]="1"\r
                        [maxFractionDigits]="2" suffix=" hrs" placeholder="e.g. 2.5 for 2h 30m"\r
                        [class.input-error]="isInvalid('estimatedHours')" styleClass="w-full" />\r
                    @if (isInvalid('estimatedHours')) {\r
                    <small class="error-msg">\r
                        @if (form.get('estimatedHours')?.hasError('required')) {\r
                        Estimated hours is required.\r
                        }\r
                        @if (form.get('estimatedHours')?.hasError('min')) {\r
                        Must be at least 0.1 hours.\r
                        }\r
                    </small>\r
                    }\r
                    <small class="form-hint">\r
                        Enter decimal hours \u2014 e.g. 1.5 = 1 hour 30 minutes\r
                    </small>\r
                </div>\r
\r
            </div>\r
\r
            <div class="form-actions">\r
                ...
                <p-button [label]="isEditMode() ? 'Update Route' : 'Create Route'" icon="pi pi-check" type="submit"\r
                    [loading]="isSaving()" />\r
            </div>\r
\r
        </form>\r
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(RouteFormComponent, { className: "RouteFormComponent", filePath: "app/features/routes/pages/route-form/route-form.component.ts", lineNumber: 32 });
})();
export {
  RouteFormComponent
};
//# sourceMappingURL=chunk-OT6B5JTU.js.map
