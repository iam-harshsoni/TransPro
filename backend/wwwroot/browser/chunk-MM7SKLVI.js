...
function toDate(value) {
  ...
  if (typeof value === "string") {
    ...
    if (/^(\d{4}(-\d{1,2}(-\d{1,2})?)?)$/.test(value)) {
      const [y4, m3 = 1, d2 = 1] = value.split("-").map((val) => +val);
      return createDate(y4, m3 - 1, d2);
    }
...
function createStyleElement(style8, doc) {
  const styleElement = doc.createElement("style");
  styleElement.textContent = style8;
  return styleElement;
}
...
var SharedStylesHost = class _SharedStylesHost {
  ...
  addHost(hostNode) {
    ...
    for (const [style8, {
      ...
      elements.push(this.addElement(hostNode, createStyleElement(style8, this.doc)));
    }
    for (const [url, {
      elements
    }] of this.external) {
...
var DefaultDomRenderer2 = class {
  ...
  setStyle(el, style8, value, flags) {
    if (flags & (RendererStyleFlags2.DashCase | RendererStyleFlags2.Important)) {
      el.style.setProperty(style8, value, flags & RendererStyleFlags2.Important ? "important" : "");
    } else {
      el.style[style8] = value;
    }
  }
  removeStyle(el, style8, flags) {
    if (flags & RendererStyleFlags2.DashCase) {
      el.style.removeProperty(style8);
    } else {
      el.style[style8] = "";
    }
  }
...
var ShadowDomRenderer = class extends DefaultDomRenderer2 {
  ...
  constructor(eventManager, hostEl, component, doc, ngZone, nonce, tracingService, sharedStylesHost) {
    ...
    for (const style8 of styles) {
      const styleEl = document.createElement("style");
      if (nonce) {
        styleEl.setAttribute("nonce", nonce);
      }
      styleEl.textContent = style8;
...
function sortAndConcatParams(params) {
  return [...params.keys()].sort().map((k5) => `${k5}=${params.getAll(k5)}`).join("&");
}
...
function fromBase64(base64) {
  ...
  const bytes = Uint8Array.from(binary, (c4) => c4.charCodeAt(0));
  return bytes.buffer;
}
...
function shallowEqualArrays(a2, b4) {
  ...
  for (let i3 = 0; i3 < a2.length; ++i3) {
    if (!shallowEqual(a2[i3], b4[i3])) return false;
  }
  return true;
}
...
function equalSegmentGroups(container, containee, matrixParams) {
  ...
  for (const c4 in containee.children) {
    if (!container.children[c4]) return false;
    if (!equalSegmentGroups(container.children[c4], containee.children[c4], matrixParams)) return false;
  }
  return true;
}
...
function containsSegmentGroupHelper(container, containee, containeePaths, matrixParams) {
  if (container.segments.length > containeePaths.length) {
    ...
    for (const c4 in containee.children) {
      if (!container.children[c4]) return false;
      if (!containsSegmentGroup(container.children[c4], containee.children[c4], matrixParams)) {
        return false;
      }
    }
    return true;
...
function equalSegments(as, bs) {
  return equalPath(as, bs) && as.every((a2, i3) => shallowEqual(a2.parameters, bs[i3].parameters));
}
function equalPath(as, bs) {
  if (as.length !== bs.length) return false;
  return as.every((a2, i3) => a2.path === bs[i3].path);
}
...
function serializeSegment(segment, root) {
  ...
  if (root) {
    ...
    const children = [];
    Object.entries(segment.children).forEach(([k5, v4]) => {
      if (k5 !== PRIMARY_OUTLET) {
        children.push(`${k5}:${serializeSegment(v4, false)}`);
      }
    });
    return children.length > 0 ? `${primary}(${children.join("//")})` : primary;
  } else {
    const children = mapChildrenIntoArray(segment, (v4, k5) => {
      if (k5 === PRIMARY_OUTLET) {
        return [serializeSegment(segment.children[PRIMARY_OUTLET], false)];
      }
      return [`${k5}:${serializeSegment(v4, false)}`];
    });
...
function mergeTrivialChildren(s4) {
  if (s4.numberOfChildren === 1 && s4.children[PRIMARY_OUTLET]) {
    const c4 = s4.children[PRIMARY_OUTLET];
    return new UrlSegmentGroup(s4.segments.concat(c4.segments), c4.children);
  }
  return s4;
}
...
function normalizeQueryParams2(k5, v4, urlSerializer) {
  k5 ||= "\u0275";
  const tree2 = new UrlTree();
  tree2.queryParams = {
    [k5]: v4
  };
  return urlSerializer.parse(urlSerializer.serialize(tree2)).queryParams[k5];
}
function tree(oldRoot, oldSegmentGroup, newSegmentGroup, queryParams, fragment, urlSerializer) {
  const qp = {};
  for (const [key, value] of Object.entries(queryParams ?? {})) {
    qp[key] = Array.isArray(value) ? value.map((v4) => normalizeQueryParams2(key, v4, urlSerializer)) : normalizeQueryParams2(key, value, urlSerializer);
  }
  let rootCandidate;
...
function replaceSegment(current, oldSegment, newSegment) {
  const children = {};
  Object.entries(current.children).forEach(([outletName, c4]) => {
    if (c4 === oldSegment) {
      children[outletName] = newSegment;
    } else {
      children[outletName] = replaceSegment(c4, oldSegment, newSegment);
    }
  });
...
function createNewSegmentGroup(segmentGroup, startIndex, commands) {
  ...
  while (i3 < commands.length) {
    ...
    if (i3 === 0 && isMatrixParams(commands[0])) {
      ...
      paths.push(new UrlSegment(p4.path, stringify2(commands[0])));
      i3++;
      continue;
    }
...
function stringify2(params) {
  const res = {};
  Object.entries(params).forEach(([k5, v4]) => res[k5] = `${v4}`);
  return res;
}
...
var Tree = class {
  ...
  siblings(t2) {
    const p4 = findPath(t2, this._root);
    if (p4.length < 2) return [];
    const c4 = p4[p4.length - 2].children.map((c5) => c5.value);
    return c4.filter((cc) => cc !== t2);
  }
  pathFromRoot(t2) {
...
function setRouterState(state, node) {
  ...
  node.children.forEach((c4) => setRouterState(state, c4));
}
function serializeNode(node) {
  const c4 = node.children.length > 0 ? ` { ${node.children.map(serializeNode).join(", ")} } ` : "";
  return `${node.value}${c4}`;
}
...
var RouterOutlet = class _RouterOutlet {
  ...
  deactivate() {
    if (this.activated) {
      const c4 = this.component;
      this.activated.destroy();
      this.activated = null;
      this._activatedRoute = null;
      this.deactivateEvents.emit(c4);
    }
  }
...
function standardizeConfig(r) {
  ...
  const c4 = children ? __spreadProps(__spreadValues({}, r), {
    children
  }) : __spreadValues({}, r);
  if (!c4.component && !c4.loadComponent && (children || c4.loadChildren) && c4.outlet && c4.outlet !== PRIMARY_OUTLET) {
    c4.component = \u0275EmptyOutletComponent;
  }
  return c4;
}
  ...
  if (prevState && routeReuseStrategy.shouldReuseRoute(curr.value, prevState.value.snapshot)) {
    ...
    if (routeReuseStrategy.shouldAttach(curr.value)) {
      ...
      if (detachedRouteHandle !== null) {
        ...
        tree2.children = curr.children.map((c4) => createNode(routeReuseStrategy, c4));
        return tree2;
      }
    }
    const value = createActivatedRoute(curr.value);
    const children = curr.children.map((c4) => createNode(routeReuseStrategy, c4));
    return new TreeNode(value, children);
  }
}
...
function createActivatedRoute(c4) {
  return new ActivatedRoute(new BehaviorSubject(c4.url), new BehaviorSubject(c4.params), new BehaviorSubject(c4.queryParams), new BehaviorSubject(c4.fragment), new BehaviorSubject(c4.data), c4.outlet, c4.component, c4);
}
...
var ActivateRoutes = class {
  ...
  activateChildRoutes(futureNode, currNode, contexts) {
    const children = nodeChildrenAsMap(currNode);
    futureNode.children.forEach((c4) => {
      this.activateRoutes(c4, children[c4.value.outlet], contexts);
      this.forwardEvent(new ActivationEnd(c4.value.snapshot));
    });
...
function getChildRouteGuards(futureNode, currNode, contexts, futurePath, checks = {
  ...
  futureNode.children.forEach((c4) => {
    getRouteGuards(c4, prevChildren[c4.value.outlet], contexts, futurePath.concat([c4.value]), checks);
    delete prevChildren[c4.value.outlet];
  });
  Object.entries(prevChildren).forEach(([k5, v4]) => deactivateRouteAndItsChildren(v4, contexts.getContext(k5), checks));
  return checks;
}
...
function runCanDeactivate(component, currARS, currRSS, futureRSS) {
  ...
  const canDeactivateObservables = canDeactivate.map((c4) => {
    const closestInjector = currARS._environmentInjector;
    const guard = getTokenOrFunctionIdentity(c4, closestInjector);
...
var ApplyRedirects = class {
  ...
  async lineralizeSegments(route, urlTree) {
    ...
    let c4 = urlTree.root;
    while (true) {
      res = res.concat(c4.segments);
      if (c4.numberOfChildren === 0) {
        return res;
      }
      if (c4.numberOfChildren > 1 || !c4.children[PRIMARY_OUTLET]) {
        throw namedOutletsRedirect(`${route.redirectTo}`);
      }
      c4 = c4.children[PRIMARY_OUTLET];
    }
  }
  ...
  createQueryParams(redirectToParams, actualParams) {
    const res = {};
    Object.entries(redirectToParams).forEach(([k5, v4]) => {
      const copySourceValue = typeof v4 === "string" && v4[0] === ":";
      if (copySourceValue) {
        const sourceName = v4.substring(1);
        res[k5] = actualParams[sourceName];
      } else {
        res[k5] = v4;
      }
    });
...
function match(segmentGroup, route, segments) {
  ...
  const posParams = {};
  Object.entries(res.posParams ?? {}).forEach(([k5, v4]) => {
    posParams[k5] = v4.path;
  });
...
function checkOutletNameUniqueness(nodes) {
  ...
  nodes.forEach((n) => {
    ...
    if (routeWithSameOutletName) {
      const p4 = routeWithSameOutletName.url.map((s4) => s4.toString()).join("/");
      const c4 = n.value.url.map((s4) => s4.toString()).join("/");
      throw new RuntimeError(4006, (typeof ngDevMode === "undefined" || ngDevMode) && `Two segments cannot have the same outlet name: '${p4}' and '${c4}'.`);
    }
...
function recognize(injector, configLoader, rootComponentType, config, serializer, paramsInheritanceStrategy, abortSignal) {
  return mergeMap(async (t2) => {
    const {
      state: targetSnapshot,
      tree: urlAfterRedirects
    } = await recognize$1(injector, configLoader, rootComponentType, config, t2.extractedUrl, serializer, paramsInheritanceStrategy, abortSignal);
...
async function loadChildren(route, compiler, parentInjector, onLoadEndListener) {
  ...
  const t2 = await maybeResolveResources(maybeUnwrapDefaultExport(loaded));
  let factoryOrRoutes;
  if (t2 instanceof NgModuleFactory$1 || Array.isArray(t2)) {
    factoryOrRoutes = t2;
  } else {
    factoryOrRoutes = await compiler.compileModuleAsync(t2);
  }
...
var RouterLink = class _RouterLink {
  ...
  _urlTree = computed(() => {
    ...
    equal: (a2, b4) => this.computeHref(a2) === this.computeHref(b4)
  }));
  get urlTree() {
...
var RouterLinkActive = class _RouterLinkActive {
  ...
  set routerLinkActive(data) {
    const classes6 = Array.isArray(data) ? data : data.split(" ");
    this.classes = classes6.filter((c4) => !!c4);
  }
  ngOnChanges(changes) {
    this.update();
  }
  ...
  update() {
    ...
    queueMicrotask(() => {
      ...
      this.classes.forEach((c4) => {
        if (hasActiveLinks) {
          this.renderer.addClass(this.element.nativeElement, c4);
        } else {
          this.renderer.removeClass(this.element.nativeElement, c4);
        }
      });
...
function D(t2, e, o = true) {
  ...
  if (t2) {
    ...
    f2.top + u2 + s4 > lt.height ? (L2 = f2.top + g2 - s4, ot = "bottom", L2 < 0 && (L2 = g2)) : L2 = u2 + f2.top + g2, f2.left + a2 > lt.width ? N3 = Math.max(0, f2.left + it + p4 - a2) : N3 = f2.left + it, V(t2) ? t2.style.insetInlineEnd = N3 + "px" : t2.style.insetInlineStart = N3 + "px", t2.style.top = L2 + "px", t2.style.transformOrigin = ot, o && (t2.style.marginTop = ot === "bottom" ? `calc(${(r = (n = x(/-anchor-gutter$/)) == null ? void 0 : n.value) != null ? r : "2px"} * -1)` : (l3 = (i3 = x(/-anchor-gutter$/)) == null ? void 0 : i3.value) != null ? l3 : "");
  }
}
function S(t2, e) {
...
function I(t2, e, o = true, n = void 0) {
  var r;
  if (t2) {
    let i3 = t2.offsetParent ? { width: t2.offsetWidth, height: t2.offsetHeight } : w(t2), l3 = e.offsetHeight, d2 = e.getBoundingClientRect(), s4 = h(), a2, u2, p4 = n != null ? n : "top";
    if (!n && d2.top + l3 + i3.height > s4.height ? (a2 = -1 * i3.height, p4 = "bottom", d2.top + a2 < 0 && (a2 = -1 * d2.top)) : a2 = l3, i3.width > s4.width ? u2 = d2.left * -1 : d2.left + i3.width > s4.width ? u2 = (d2.left + i3.width - s4.width) * -1 : u2 = 0, t2.style.top = a2 + "px", t2.style.insetInlineStart = u2 + "px", t2.style.transformOrigin = p4, o) {
...
function de(e, t2 = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u2 in m3) {
      let g2 = m3[u2], f2 = ve(g2), p4 = G(u2, a2) ? Q2(d2) : Q2(d2, re(u2));
...
var b3 = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  var R2, T3, j3, O2, M2, z4, V3;
  let { preset: a2, options: n } = t2, l3, o, c4, m3, d2, u2, g2;
  if (s2(a2) && n.transform !== "strict") {
    let { primitive: L2, semantic: te2, extend: re2 } = a2, f2 = te2 || {}, { colorScheme: K2 } = f2, A3 = v2(f2, ["colorScheme"]), x3 = re2 || {}, { colorScheme: X2 } = x3, G3 = v2(x3, ["colorScheme"]), p4 = K2 || {}, { dark: U3 } = p4, B = v2(p4, ["dark"]), y4 = X2 || {}, { dark: I2 } = y4, H3 = v2(y4, ["dark"]), W3 = s2(L2) ? this._toVariables({ primitive: L2 }, n) : {}, q3 = s2(A3) ? this._toVariables({ semantic: A3 }, n) : {}, Z = s2(B) ? this._toVariables({ light: B }, n) : {}, pe = s2(U3) ? this._toVariables({ dark: U3 }, n) : {}, fe = s2(G3) ? this._toVariables({ semantic: G3 }, n) : {}, ye = s2(H3) ? this._toVariables({ light: H3 }, n) : {}, Se = s2(I2) ? this._toVariables({ dark: I2 }, n) : {}, [Me, ze] = [(R2 = W3.declarations) != null ? R2 : "", W3.tokens], [Ke, Xe] = [(T3 = q3.declarations) != null ? T3 : "", q3.tokens || []], [Ge, Ue] = [(j3 = Z.declarations) != null ? j3 : "", Z.tokens || []], [Be, Ie] = [(O2 = pe.declarations) != null ? O2 : "", pe.tokens || []], [He, We] = [(M2 = fe.declarations) != null ? M2 : "", fe.tokens || []], [qe, Ze] = [(z4 = ye.declarations) != null ? z4 : "", ye.tokens || []], [Fe, Je] = [(V3 = Se.declarations) != null ? V3 : "", Se.tokens || []];
  ...
  if (s2(t2) && r.transform !== "strict") {
    let y4 = e.replace("-directive", ""), m3 = t2, { colorScheme: R2, extend: T3, css: j3 } = m3, O2 = v2(m3, ["colorScheme", "extend", "css"]), d2 = T3 || {}, { colorScheme: M2 } = d2, z4 = v2(d2, ["colorScheme"]), u2 = R2 || {}, { dark: V3 } = u2, L2 = v2(u2, ["dark"]), g2 = M2 || {}, { dark: te2 } = g2, re2 = v2(g2, ["dark"]), K2 = s2(O2) ? this._toVariables({ [y4]: h2(h2({}, O2), z4) }, r) : {}, A3 = s2(L2) ? this._toVariables({ [y4]: h2(h2({}, L2), re2) }, r) : {}, X2 = s2(V3) ? this._toVariables({ [y4]: h2(h2({}, V3), te2) }, r) : {}, [G3, U3] = [(f2 = K2.declarations) != null ? f2 : "", K2.tokens || []], [B, I2] = [(x3 = A3.declarations) != null ? x3 : "", A3.tokens || []], [H3, W3] = [(p4 = X2.declarations) != null ? p4 : "", X2.tokens || []], q3 = this.transformCSS(y4, `${G3}${B}`, "light", "variable", r, i3, a2, n), Z = this.transformCSS(y4, H3, "dark", "variable", r, i3, a2, n);
  ...
  let a2 = function(l3, o = {}, c4 = []) {
    ...
    Object.entries(l3).forEach(([m3, d2]) => {
      ...
      i2(d2) ? n(d2, u2, g2) : (i3[u2] || (i3[u2] = { paths: [], computed: (f2, x3 = {}, p4 = []) => {
...
var DomHandler = class _DomHandler {
  ...
  static fadeIn(element, duration) {
    ...
    let opacity = 0;
    let tick = function() {
      opacity = +element.style.opacity.replace(",", ".") + ((/* @__PURE__ */ new Date()).getTime() - last2) / duration;
  ...
  static getViewport() {
    let win = window, d2 = document, e = d2.documentElement, g2 = d2.getElementsByTagName("body")[0], w3 = win.innerWidth || e.clientWidth || g2.clientWidth, h3 = win.innerHeight || e.clientHeight || g2.clientHeight;
...
function tt(t2, n) {
  ...
  let e = {}, o = false, m3 = {}, i3 = null, M2 = {}, f2 = (a2) => {
    ...
    i3 == null || i3();
    let { onBefore: s4, onStart: d2, onAfter: l3, onCancelled: c4 } = M2[a2] || {}, h3 = { element: t2 };
    if (o) {
      s4 == null || s4(h3), d2 == null || d2(h3), l3 == null || l3(h3);
      return;
    }
    let { from: g2, active: y4, to: P2 } = m3[a2] || {};
    return S3(t2, e.autoHeight, e.autoWidth), s4 == null || s4(h3), W(t2, g2), W(t2, y4), t2.offsetHeight, P(t2, g2), W(t2, P2), d2 == null || d2(h3), new Promise((b4) => {
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-MM7SKLVI.js.map
